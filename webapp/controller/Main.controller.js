sap.ui.define([
	...
	"../model/formatter",
	'sap/m/library'
], function (Controller, JSONModel, MessageBox, Filter, FilterOperator, MessageToast, Token, formatter, mobileLibrary) {
	"use strict";
	var FormItem = [],
		getMeinsSH = [],
		Vekalet = [],
	...
	return Controller.extend("app.Satici.controller.Main", {
		formatter: formatter,
		...
		_onRouteMatched: function () {
			...
			this.getView().getModel("InputModel").setProperty("/InputModelEdit", false);
		...
		onCreateTalep: function (oEvent) {
			...
			if (ModelForm[0].Bukrs === "") {
				MessageBox.warning("Şirket kodu bulunamadı.");
				return;
				ctrl = 1;
			}
			if (ModelForm[0].Ekorg === "") {
				ctrl = 1;
				MessageBox.warning("Satınalma Organizasyonu bulunamadı.");
				return;
			}
			if (ModelForm[0].Ktokk === "") {
				ctrl = 1;
				MessageBox.warning("Hesap grubu alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].Ktock === "") {
				ctrl = 1;
				MessageBox.warning("Referans alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].Name1 === "") {
				ctrl = 1;
				MessageBox.warning("Ad alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].Telf1 === "") {
				ctrl = 1;
				MessageBox.warning("Telefon alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].AdSmtpadr === "") {
				ctrl = 1;
				MessageBox.warning("E-posta alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].Waers === "") {
				ctrl = 1;
				MessageBox.warning("SAS Para Birimi alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].Saknr === "") {
				ctrl = 1;
				MessageBox.warning("Mutabakat Hesabı alanını doldurunuz.");
				return;
			}
			if (ModelForm[0].Tolerans === "") {
				ctrl = 1;
				MessageBox.warning("Tolerans Seçiniz.");
				return;
			}
			if (ctrl === 0) {
				MessageBox.confirm("Talebi oluşturmak istiyor musunuz?", {
					...
					onClose: function (sButton) {
						if (sButton === MessageBox.Action.OK) {
							...
							ModelForm[0].Ekorg = ModelForm[0].Ekorg.toString();
							ModelForm[0].Ktokk = ModelForm[0].Ktokk.toString();
							ModelForm[0].Ktock = ModelForm[0].Ktock.toString();
							ModelForm[0].Name1 = ModelForm[0].Name1.toString();
							ModelForm[0].Telf1 = ModelForm[0].Telf1.toString();
							ModelForm[0].Waers = ModelForm[0].Waers.toString();
							ModelForm[0].Saknr = ModelForm[0].Saknr.toString();
							ModelForm[0].Tolerans = ModelForm[0].Tolerans.toString();
							ModelForm[0].AdSmtpadr = ModelForm[0].AdSmtpadr[0].toString();
							...
							var obj = {
								Requestid: reqid,
								Bukrs: Sirket,
								Ekorg: that.getView().byId("DepS").getValue(),
								Ktokk: that.getView().byId("Meins").getValue(),
								Ktock: that.getView().byId("Gblok").getValue(),
								Name1: that.getView().byId("Gblo").getValue(),
								Telf1: that.getView().byId("Gbl").getValue(),
								AdSmtpadr: that.getView().byId("Gb").getValue(),
								Stcd1: "",
								Stkzu: that.getView().byId("idstkzu").getSelected(),
								Land1: "",
								Bankk: "",
								Bankn: "",
								KoinhFi: "",
								...
								Ereturnt: "",
								Vekalet: "",
								Waers: that.getView().byId("idSSASParaBirimi").getValue(),
								Saknr: that.getView().byId("idMutabakat").getValue(),
								Tolerans: that.getView().byId("idTolerans").getSelectedKey()

							};
							selectedItems1.push(obj);
							// requestHeader = ModelForm;
							// requestHeader.NAVISDATA = ModelForm;

							requestHeader.NAVISDATA = selectedItems1;
							...
							oDataModel.create("/YeniTalepKayitCreateDeepSet", requestHeader, {
							...
							function mySuccessHandler(data, response) {
								...
								MessageBox.success(msg);
								that.getView().byId("Meins").setValue("");
								that.getView().byId("Gblo").setValue("");
								that.getView().byId("Gbl").setValue("");
								that.getView().byId("Gb").setValue("");
								that.getView().byId("idDosyaButton1").setIcon("sap-icon://add");
								that.getView().byId("FormDisplay480_12120Dual").setVisible(false);
				...
				MessageBox.confirm("Talebi onaylamak istiyor musunuz?", {
					...
					onClose: function (sButton) {
						if (sButton === MessageBox.Action.OK) {
							...
							if (Yetki === "02") {
								aSelectedItems[0].Statu = "02";
								if (aSelectedItems[0].Stcd1 === "") {
									MessageBox.warning("Vergi numarası alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Stkzu === "") {
									MessageBox.warning("KDV alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Land1 === "") {
									MessageBox.warning("Ülke alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Bankk === "") {
									MessageBox.warning("Banka anahtarı alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Bankn === "") {
									MessageBox.warning("Banka hesabı alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Bankn === "") {
									MessageBox.warning("Banka hesabı alanını doldurunuz.");
								...
								if (aSelectedItems[0].KoinhFi === "") {
									MessageBox.warning("Hesap sahibi alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Bvtyp === "") {
									MessageBox.warning("Banka tipi alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Altkn === "") {
									MessageBox.warning("Eski hesap no alanını doldurunuz.");
									return;
								}
								if (aSelectedItems[0].Zterm === "") {
									MessageBox.warning("Ödeme koşulu alanını doldurunuz.");
									return;
								}
								/*	aSelectedItems[0].EtiketBilgi = aSelectedItems[0].EtiketTemp.slice(0, 1332);
									aSelectedItems[0].EtiketBilgi2 = aSelectedItems[0].EtiketTemp.slice(1333, 2665);
		...
		_getDialogRevize: function () {
			if (!this._oDialogRevize) {
				this._oDialogRevize = sap.ui.xmlfragment("app.Satici.view.fragments.RevizeNeden", this);
		...
		onCreateRevizeTamamla: function (oEvent) {
			...
			if (aSelectedIndices.length === 0) {
				...
				if (aSelectedItems[0].Ktokk === "") {
					ctrl = 1;
					MessageBox.warning("Hesap grubu alanını doldurunuz.");
					return;
				}
				if (aSelectedItems[0].Ktock === "") {
					ctrl = 1;
					MessageBox.warning("Referans alanını doldurunuz.");
					return;
				}
				if (aSelectedItems[0].Name1 === "") {
					ctrl = 1;
					MessageBox.warning("Ad alanını doldurunuz.");
					return;
				}
				if (aSelectedItems[0].Telf1 === "") {
					ctrl = 1;
					MessageBox.warning("Telefon alanını doldurunuz.");
					return;
				}
				if (aSelectedItems[0].AdSmtpadr === "") {
					ctrl = 1;
					MessageBox.warning("E-posta alanını doldurunuz.");
					return;
				}
				if (aSelectedItems[0].Waers === "") {
					MessageBox.warning("SAS Para Birimi seçiniz.");
					return;
					ctrl = 1;
				}
				if (aSelectedItems[0].Saknr === "") {
					MessageBox.warning("Mutabakat Hesabı seçiniz.");
					return;
					ctrl = 1;
				}
				if (aSelectedItems[0].Tolerans === "") {
					MessageBox.warning("Tolerans seçiniz.");
					return;
					ctrl = 1;
				}
				MessageBox.confirm("Revizeyi tamamlamak istiyor musunuz?", {
					...
					onClose: function (sButton) {
						if (sButton === MessageBox.Action.OK) {
							...
							delete aSelectedItems[0].Onay;
							delete aSelectedItems[0].Neden;
							...
							aSelectedItems[0].Onay = "";
							aSelectedItems[0].Neden = "";
		...
		_getDialogRet: function () {
			if (!this._oDialogRet) {
				this._oDialogRet = sap.ui.xmlfragment("app.Satici.view.fragments.RedNeden", this);
		...
		onActionPressedVekalet: function (oEvent) {
			var that = this;
			var item = oEvent.getSource();
			var sAction = item.getCustomData()[0].getValue();
			that.GetRoleVekalet(sAction);
			// var sServiceURL = this.getView().getModel().sServiceUrl;
			// URLHelper.redirect(sAction, true);
			// window.open(sAction, "_blank");
		},
		...
		onUploadComplete: function (e) {
			...
			sap.ui.getCore().byId("uploadColIkDokuman").addItem(item);
			this.getView().byId("idDosyaButton1").setIcon("sap-icon://accept");
			this.getView().byId("FormDisplay480_12120Dual").setVisible(true);
		...
		onChange: function (oEvent) {
			...
			var uniqeFormno = reqid;
			var url = "/sap/opu/odata/sap/ZXK_01_V1_SRV/FileSet(IvGuid='" + uniqeGuid + "',IvFormno='" + uniqeFormno + "')/FILENP";
			t.setUploadUrl(url);
			// urlll = url;
		},
		GetRole: function () {
			...
			oDataModel.read("/GetRoleSet(Uname='" + userData.Username + "')", {
				success: mySuccessHandler,
				error: myErrorHandler
			});
			...
			function mySuccessHandler(data, response) {
				...
				that.getView().byId("MagS").setValue(Sirket);

				// sap.ui.getCore().byId("vekalet").setValue(data.Aciklama);

				that.getView().byId("DepS").setValue(data.Ekorg);
				that.VisibleTab();
				sap.ui.core.BusyIndicator.hide();
			}
		...
		GetRoleVekalet: function (sAction) {
			var that = this;
			var oDataModel = this.getOwnerComponent().getModel();

			sap.ui.core.BusyIndicator.show();

			oDataModel.read("/GetRoleSet(Uname='" + sAction + "')", {
				success: mySuccessHandler,
				error: myErrorHandler
			});
			...
			function mySuccessHandler(data, response) {
				...
				// Vekalet.push(obj);
				that.getView().getModel("InputModel").setProperty("/Sirket", Sirket);
				MessageToast.show("Vekalet Kullanıcısına Geçildi");
				that.onClose3();
				// that.getView().byId("MagS").setValue(Sirket);
		...
		onValueHelpUlke: function (oEvent) {
			...
			if (!this._ulkeSH) {
				this._ulkeSH = sap.ui.xmlfragment("app.Satici.view.fragments.UlkeSH", this);
				oModel.addDependent(this._ulkeSH);
				this._ulkeSH.open();
			} else {
				oModel.addDependent(this._ulkeSH);
				this._ulkeSH.open();
			}
			//  secilisatir = 	oEvent.getSource().getBindingContext("oViewModel");
		},
		onValueHelpKtokk: function (oEvent) {
			// var Index = oEven.getSource().getParent().getCells()[0].mProperties.text;
			// TableIndex = this.t.getSource().getParent().sId.split("row").length;
			// var TalNo = oEventgetView().getModel("oViewModel").getData().taleOnay.findIndex(x => x.Requestid === TalNo);

			var oModel = this.getView();
			if (!this._ktokkSH) {
				this._ktokkSH = sap.ui.xmlfragment("app.Satici.view.fragments.KtokkSH", this);
				oModel.addDependent(this._ktokkSH);
				this._ktokkSH.open();
			} else {
				oModel.addDependent(this._ktokkSH);
				this._ktokkSH.open();
		...
		_searchOdemeVadesiSH: function (oEvent) {

			var oFilter = [];
			var sValue = oEvent.getParameter("value");
			oFilter = new Filter("IvSearchField", FilterOperator.Contains, sValue);
			oEvent.getSource().getBinding("items").filter([oFilter]);
		},
		_confirmOdemeVadesiSH: function (e) {
			var t = e.getParameter("selectedContexts");
			if (t && t.length) {
				var s = t[0].getProperty("Zterm");
				var g = t[0].getProperty("Text1");
				e.getSource().getBinding("items").filter([]);
				this.byId("Zterm").setValue(s);
				this.getView().getModel("oViewModel").getData().taleOnay[TableIndex].Zterm = s;
				this.getView().getModel("oViewModel").refresh(true);

				// this.byId("idMOdemeVadesii").setValue(g);
			}
		},
		_searchUlkeSH: function (oEvent) {
			...
			// } else {
			// 	oFilter = new Filter("Landx", FilterOperator.Contains, sValue);
			// }
			oEvent.getSource().getBinding("items").filter(oFilter);
		},
		_searchKtokkSH: function (oEvent) {
		...
		_confirmUlkeSH: function (e) {

			var t = e.getParameter("selectedContexts");
			if (t && t.length) {
				var s = t[0].getProperty("Land1");
				var g = t[0].getProperty("Landx");
				e.getSource().getBinding("items").filter([]);
				// this.getView().byId("Ulke").setValue(s);
				// var eklenecekLand1 = oEvent.getSource().getBindingContext().getObject();
				this.getView().getModel("oViewModel").getData().taleOnay[TableIndex].Land1 = s;
				this.getView().getModel("oViewModel").refresh(true);
				// this._MeinsValueHelpRev.destroy();

				// this.byId("idUlkee").setValue(g);
				// sFilterValueSehir = s;
				// secilisatir.setProperty("Land1",s);
				// this.getView().byId("Ulke")
			}
		},
		_confirmKtokkSH: function (e) {

			var t = e.getParameter("selectedContexts");
			if (t && t.length) {
				var s = t[0].getProperty("Ktokk");
				// var g = t[0].getProperty("Landx");
				e.getSource().getBinding("items").filter([]);
				this.getView().byId("Meins").setValue(s);
				// var eklenecekLand1 = oEvent.getSource().getBindingContext().getObject();
				// this.getView().getModel("oViewModel").getData().taleOnay[TableIndex].Land1 = s;
				// this.getView().getModel("oViewModel").refresh(true);
				// this._MeinsValueHelpRev.destroy();
		...
		onValueHelpZterm: function (oEvent) {
			var Index = oEvent.getSource().getParent().sId.split("row").length;
			var TalNo = oEvent.getSource().getParent().getCells()[0].mProperties.text;
			TableIndex = this.getView().getModel("oViewModel").getData().taleOnay.findIndex(x => x.Requestid === TalNo);

			var oModel = this.getView();
			if (!this._odemevadesiSH) {
				this._odemevadesiSH = sap.ui.xmlfragment("app.Satici.view.fragments.OdemeVadesiSH", this);
				oModel.addDependent(this._odemevadesiSH);
				this._odemevadesiSH.open();
			} else {
				oModel.addDependent(this._odemevadesiSH);
				this._odemevadesiSH.open();
			}
		...
		_confirmSASParaBirimi: function (e) {
			var t = e.getParameter("selectedContexts");
			if (t && t.length) {
				var s = t[0].getProperty("Waers");
				var g = t[0].getProperty("Ltext");

				e.getSource().getBinding("items").filter([]);
				this.byId("idSSASParaBirimi").setValue(s);
				// this.byId("idSSASParaBirimi").setValue(g);

			}
		},
		_confirmMutabakat: function (e) {
			var t = e.getParameter("selectedContexts");
			if (t && t.length) {
				var s = t[0].getProperty("Saknr");
				// var g = t[0].getProperty("Ltext");

				e.getSource().getBinding("items").filter([]);
				this.byId("idMutabakat").setValue(s);
				// this.byId("idSSASParaBirimi").setValue(g);
		...
		onDosyaYukle: function () {
			...
			if (!this.excelFrag) {
				this.excelFrag = sap.ui.xmlfragment("app.Satici.view.fragments.docupload", this);
		...
		onPressDosya: function (oEvent) {
			var IstekNo = oEvent.getSource().getBindingContext("oViewModel").getProperty("Requestid")
			...
			if (!this.dosya) {
				this.dosya = sap.ui.xmlfragment("app.Satici.view.fragments.Dosya", this);
		...
		onVekalet: function (oEvent) {
			...
			if (!this.vekalet) {
				this.vekalet = sap.ui.xmlfragment("app.Satici.view.fragments.Vekalet", this);
				this.getView().addDependent(this.vekalet);
			}
			var oVekaletModel = new sap.ui.model.json.JSONModel();
			oVekaletModel.setData({
				Vekalet: Vekalet
			});
			this.getView().setModel(oVekaletModel, "Vekalet");
			this.getView().getModel("Vekalet").refresh(true);
			this.vekalet.open();
			// sap.ui.getCore().byId("vekalet").setText(Vekalet);
			// for(var i = 0; i<sepet.length; i++ ){
		...
		onBeforeRebindTable: function (oEvent) {
			...
			var IStatu = Yetki,
				...
				oFilter;
			if (IStatu) {
				oFilter = new sap.ui.model.Filter("IStatu", sap.ui.model.FilterOperator.EQ, "S");
		...
		onTabbar: function (oEvent) {
			...
			if (oEvent.getSource().mProperties.selectedKey === "Ttalep") {
				...
				this.getView().getModel("InputModel").setProperty("/InputModelButtonYarat", false);
				setTimeout(function () {
					this.getView().byId("smartCMMGiris").rebindTable();
				}.bind(this), 100);
			} else {
				this.getView().getModel("InputModel").setProperty("/InputModelButtonTalep", false);
			}
		...
		onValueHelpSASParaBirimi: function () {
			var oModel = this.getView();
			if (!this._sasparaSH) {
				this._sasparaSH = sap.ui.xmlfragment("app.Satici.view.fragments.SASParaBirimiSH", this);
				oModel.addDependent(this._sasparaSH);
				this._sasparaSH.open();
			} else {
				oModel.addDependent(this._sasparaSH);
				this._sasparaSH.open();
			}
		},
		onValueHelpMutabakat: function () {
			var oModel = this.getView();
			if (!this._mtkbktSH) {
				this._mtkbktSH = sap.ui.xmlfragment("app.Satici.view.fragments.MtkbkSH", this);
				oModel.addDependent(this._mtkbktSH);
				this._mtkbktSH.open();
			} else {
				oModel.addDependent(this._mtkbktSH);
				this._mtkbktSH.open();
		...
		_searchSASParaBirimi: function (oEvent) {
